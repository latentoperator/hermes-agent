...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._turn_index = 0
        self._client = None
        # queue_prefetch(), retain writer work, and memory tool calls can all
        # reach _get_client() from different threads. Without a creation lock,
        # two first-use callers can each build an aiohttp-backed client; the
        # last assignment wins and the orphaned client is never closed.
    ...
    def _get_client(self):
        ...
        with self._client_lock:
            # Double-check after acquiring the lock: another prefetch/tool/
            # writer thread may have completed first-use initialization while
            # this caller waited.
    ...
    def _create_client(self):
        """Build one Hindsight client while ``_client_lock`` is held."""
