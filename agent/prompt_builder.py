...
KANBAN_GUIDANCE = (
    ...
    "files outside it unless the task explicitly asks. Scratch workspaces are "
    "working desks, not archives: before completion, copy important reports, "
    "manifests, hash inventories, rollback notes, and verification checklists "
    "to durable paths outside scratch (repo, shared dir, Obsidian, etc.). For "
    "`worktree` tasks, the branch is the durable artifact: if `.git` is "
    "missing, create/reuse the worktree from the project repo, push the branch "
    "on creation (`git push -u origin <branch>`), then commit as soon as "
    "meaningful work exists and push right after. Worktrees are isolated desks, "
    "not storage.\n"
    ...
    "(`{changed_files: [...], tests_run: N, decisions: [...]}`). For code "
    "changes, include git evidence: branch, base SHA, HEAD SHA, clean "
    "`git status --porcelain`, freshly fetched/remote `origin/<branch>` HEAD, "
    "and the representative environment used for verification. Downstream "
    ...
    "- **Deliverables.** Files a human wants delivered in the completion "
    "notification go in `kanban_complete(artifacts=[<absolute paths>])` "
    "(top-level param; paths in `metadata` are NOT uploaded). This is "
    "delivery, not durability: missing files are skipped by notifiers, and a "
    "scratch-only artifact path is acceptable only for ephemeral immediate "
    "upload files. If a report, manifest, hash inventory, rollback note, or "
    "verification checklist must be useful after completion, save a durable "
    "copy first and point `artifacts`/metadata at that durable path.\n"
