...
KANBAN_GUIDANCE = (
    ...
    "files outside it unless the task explicitly asks. For `worktree` tasks, "
    "the branch is the durable artifact: if `.git` is missing, create/reuse "
    "the worktree from the project repo, push the branch on creation "
    "(`git push -u origin <branch>`), then commit as soon as meaningful work "
    "exists and push right after. Worktrees are isolated desks, not storage.\n"
    ...
    "Exception: if your output is a code change that needs code review "
    "before merge, deploy, or production/client handoff, drop the "
    "structured metadata (repo/path, branch or PR, base/head SHA, "
    "changed_files, tests_run, and remote status) into a `kanban_comment` "
    "first, then end with "
    "`kanban_block(reason=\"review-required: <one-line summary>\")` so the "
    "review loop can assign the code-reviewer, approve+unblock, request "
    "changes, or escalate. Do NOT use `review-required` reflexively for "
    "trivial docs, formatting, comments, generated reports, scratch scripts, "
    "or test-only cleanup; use it for runtime/security/data-integrity/"
    "cross-agent/production-facing changes, PRs that need approval before "
    "merge, deployments that need signoff, or cases where uncertainty remains.\n"
