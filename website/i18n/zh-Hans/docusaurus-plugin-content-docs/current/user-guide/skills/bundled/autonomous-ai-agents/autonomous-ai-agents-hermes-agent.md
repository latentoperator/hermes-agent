...
- **Worker toolset：** 调度器生成的任务可获得 `kanban_show`、`kanban_complete`、`kanban_block`、`kanban_heartbeat`、`kanban_comment`、`kanban_create`、`kanban_link` 以及 worker 执行协议。无任务且显式启用 `kanban` 的 profile 仅获得路由工具：`kanban_list`、`kanban_show`、`kanban_comment`、`kanban_create`、`kanban_link` 和 `kanban_unblock`。
