...
Registered when the agent is either (a) spawned by the kanban dispatcher (`HERMES_KANBAN_TASK` env set) or (b) running in a profile that explicitly enables the `kanban` toolset. Task-scoped workers get the lifecycle protocol and worker tools. Taskless orchestrator sessions get only the routing surface (`kanban_list`, `kanban_show`, `kanban_comment`, `kanban_create`, `kanban_link`, and `kanban_unblock`) — not worker guidance or lifecycle mutations. See [Kanban Multi-Agent](/user-guide/features/kanban) for the full workflow.
