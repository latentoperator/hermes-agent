...
- **`cron.follow_profile_defaults: true`** — explicitly follow the profile’s current model and provider on each run, ignoring creation snapshots. Per-job and `cron.model` / `cron.model_provider` pins still win. An existing explicit `cron.model_drift_guard: false` migrates to this policy.
