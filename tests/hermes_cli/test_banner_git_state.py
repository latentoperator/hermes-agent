...
def test_local_update_check_fetches_official_upstream_for_fork(tmp_path):
    from hermes_cli import banner

    def fake_stdout(args, **_kwargs):
        values = {
    ...
    def fake_run(cmd, **_kwargs):
        if cmd[:4] == ["git", "fetch", "upstream", "main"]:
            return MagicMock(returncode=0, stdout="")
        if cmd == ["git", "rev-list", "--count", "HEAD..upstream/main"]:
            return MagicMock(returncode=0, stdout="7\n")
        raise AssertionError(f"unexpected command: {cmd}")

    with patch.object(banner, "_git_stdout", side_effect=fake_stdout), patch(
        "hermes_cli.banner.subprocess.run", side_effect=fake_run
    ):
