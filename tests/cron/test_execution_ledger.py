...
def test_profile_scoped_delivery_hydrates_external_secrets(monkeypatch, tmp_path):
    """BWS/other external aliases must be hydrated before delivery scope build."""
    import agent.secret_scope as secret_scope
    import cron.scheduler as scheduler
    ...
    monkeypatch.setattr(
        ...
        lambda home: hydrated.append(home) or {"TELEGRAM_BOT_TOKEN": "syren-token"},
    )
    monkeypatch.setattr(
        secret_scope,
    ...
    assert scheduler._deliver_result_in_profile_scope(
        {"id": "job-external-secret"},
        "response",
    ) is None
    assert seen["token"] == "syren-token"
    assert secret_scope.current_secret_scope() is None
