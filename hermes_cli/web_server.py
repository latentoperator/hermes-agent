...
def _ws_host_origin_reason(ws: "WebSocket") -> Optional[str]:
    ...
    # while rewriting Host to loopback. Keep the default same-bound-host guard,
    # but allow operators to name explicit public dashboard origins.
    allowed_origins = {
        item.strip().rstrip("/")
...
def _ws_request_is_allowed(ws: "WebSocket") -> bool:
    ...
    return _ws_request_reason(ws) is None
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    try:
        chat_kwargs = {"resume": resume, "sidecar_url": sidecar_url}
        if profile is not None:
            chat_kwargs["profile"] = profile
        argv, cwd, env = _resolve_chat_argv(**chat_kwargs)
