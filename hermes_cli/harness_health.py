...
FUNNEL_STAGES = (
    "Available", "Eligible", "Shown", "Consulted", "Acted through",
    "Checked", "Accepted",
)
...
def _isolated_requirement_home():
    """Keep requirement probes away from the audited profile's writable state."""
...
def _portable(path: Path, *, home: Path, cwd: Path, project_root: Path | None = None) -> str:
    path = path.resolve()
    for base, token in ((project_root, "$PROJECT_ROOT"), (home, "$HERMES_HOME"), (cwd, "$CWD")):
        if base is None:
            continue
        try:
...
def _skill_inventory(platform: str, home: Path) -> dict[str, Any]:
    ...
    available_names = sorted({str(s.get("name")) for s in available if s.get("name")})
...
def _tool_inventory(config: dict[str, Any], platform: str) -> dict[str, Any]:
    # Use the same effective platform resolver and registry entries as runtime,
    # but discover built-ins under a disposable HERMES_HOME so import-time
    # initializers cannot seed or repair the audited profile. Plugin startup and
    # credential-store probes remain disabled by contract.
    ...
    with _isolated_requirement_home():
        ...
        toolsets = sorted(_get_platform_tools(
            config,
            platform,
...
def _plugin_inventory(config: dict[str, Any]) -> dict[str, Any]:
    ...
    disabled = {str(name) for name in disabled_raw} if isinstance(disabled_raw, list) else set()
    enabled = ({str(name) for name in enabled_raw} if isinstance(enabled_raw, list) else set()) - disabled
    return {
        "state": "VERIFIED",
...
def _config_inaccessible_setup_sections() -> dict[str, dict[str, Any]]:
    return {
        ...
        "safeguards": {
            ...
            # Importing approval state can initialize config-dependent runtime
            # modules. When config itself is inaccessible, fail closed rather
            # than touching that path and risking recovery writes.
...
def _approval_runtime_overrides() -> tuple[bool | None, bool | None]:
    """Return canonical process/current-session YOLO state without reading raw env."""
    try:
...
def _normalize_approval_mode(value: Any) -> str:
    try:
        ...
        if isinstance(value, str) and value.strip().lower() in {"manual", "smart", "off"}:
...
def _safeguards(config: dict[str, Any], home: Path) -> dict[str, Any]:
    ...
    config_mode = _normalize_approval_mode(configured_mode if configured_mode is not None else "manual")
    ...
    result: dict[str, Any] = {
        ...
        "configured_approval_mode": config_mode if configured_mode is not None else None,
...
def _runtime_context(prompt: str) -> dict[str, Any]:
    """Describe only the context source proven present in the persisted prompt."""
...
def _sqlite_signature(db: Path) -> tuple[tuple[int, int] | None, tuple[int, int] | None]:
...
def _sqlite_read_snapshot(db: Path):
    """Copy SQLite state to scratch so read-only WAL access cannot write target SHM."""
    ...
    with tempfile.TemporaryDirectory(prefix="hermes-harness-health-db-") as scratch:
        ...
        if _sqlite_signature(db) != before:
            raise OSError("session database changed while creating read-only snapshot")
        yield snapshot


def _read_runtime(home: Path, cwd: Path, session_id: str | None) -> tuple[dict[str, Any], list[str]]:
    runtime = {
        "state": "INACCESSIBLE",
        "context": {},
        ...
        with _sqlite_read_snapshot(db) as snapshot_db:
            ...
            with sqlite3.connect(uri, uri=True) as conn:
                ...
                if not {"id", "system_prompt"}.issubset(session_cols) or not {
                    "session_id", "role", "tool_calls", "tool_name", "tool_call_id", "content", "active"
                }.issubset(message_cols):
                    raise sqlite3.DatabaseError("required session columns unavailable")
                select_cols = "system_prompt, cwd" if "cwd" in session_cols else "system_prompt, NULL"
                row = conn.execute(f"SELECT {select_cols} FROM sessions WHERE id = ?", (session_id,)).fetchone()
                if row is None:
        ...
        for call in calls if isinstance(calls, list) else []:
            ...
            if name == "skill_view":
                try:
                    ...
                    call_id = call.get("id") if isinstance(call, dict) else None
                    if (isinstance(skill, str) and _SKILL_NAME_RE.fullmatch(skill)
    ...
    runtime = {
        ...
        "skills": {"shown_names": _safe_names(shown), "preloaded_names": _safe_names(preloaded),
                   "viewed_names": _safe_names(viewed), "loaded_names": _safe_names(preloaded | viewed)},
        "tools": {
    ...
    if plugin_state == "INFERRED":
        blind.append("Plugin ownership inferred from the plugin-* toolset naming convention.")
    elif not plugin_used:
        blind.append("Historical plugin tool ownership is inaccessible unless loaded registry ownership proves it.")
    if not mcp_used:
        blind.append("Historical MCP tool ownership is inaccessible unless registry ownership proves it.")
    if unknown_ownership:
        blind.append("Some historical tool names are no longer registered, so ownership is inaccessible.")
    return runtime, blind


def _funnel(setup: dict[str, Any], runtime: dict[str, Any]) -> dict[str, Any]:
    ...
    if runtime["state"] == "INACCESSIBLE":
        ...
        # A persisted call proves the capability passed each upstream stage at
        # run time, even though Hermes did not persist those upstream snapshots.
        values = (acted, acted, acted, acted, acted, checked, 0)
        states = (
            "INFERRED", "INFERRED", "INFERRED", "INFERRED",
            "VERIFIED", "VERIFIED", "INACCESSIBLE",
        )
    funnel = {
...
def collect_harness_health(*, platform: str = "cli", cwd: str | Path | None = None,
    ...
    try:
        ...
        config_blind.append(
            f"Profile config is inaccessible ({type(exc).__name__}); config-derived setup is inaccessible."
        )
    config_sections = (
        {
            "tools": _tool_inventory(config, platform),
            "plugins": _plugin_inventory(config),
            "mcp": _mcp_inventory(config),
            "safeguards": _safeguards(config, home),
        }
        if config_state == "VERIFIED"
        else _config_inaccessible_setup_sections()
    )
    ...
    report = {
        "schema_version": "0.1", "evidence_states": list(EVIDENCE_STATES),
        "profile": {"identity": _identity(home)},
        "scope": {"platform": _safe_name(platform), "cwd": "$CWD",
                  "session_identity": _text_identity(session_id) if session_id else None},
        "setup": setup, "runtime": runtime,
        "funnel": _funnel(setup, runtime), "blind_spots": blind,
    }
    return report


def write_report_exclusive(report: dict[str, Any], destination: str | Path) -> None:
    """Write JSON using exclusive creation; an existing destination is never changed."""
...
def render_report(report: dict[str, Any]) -> str:
    ...
    return "\n".join((
        ...
        f"Funnel: {report['funnel']['completeness']} ({len(report['blind_spots'])} blind spots)",
    ))


def cmd_harness_health(args: Any) -> None:
    report = collect_harness_health(platform=args.platform, cwd=args.cwd, session_id=args.session)
    if args.output:
